...
class Data(object):
    ...
    def get_adj_mat(self):
        try:
            ...
            adj_mat, norm_adj_mat, mean_adj_mat, sym_adj_mat = self.create_adj_mat()
            sp.save_npz(self.path + '/s_adj_mat.npz', adj_mat)
            sp.save_npz(self.path + '/s_norm_adj_mat.npz', norm_adj_mat)
            sp.save_npz(self.path + '/s_mean_adj_mat.npz', mean_adj_mat)
            sp.save_npz(self.path + '/s_sym_adj_mat.npz', sym_adj_mat)

        return adj_mat, norm_adj_mat, mean_adj_mat, sym_adj_mat
    ...
    def create_adj_mat(self):
        ...
        def normalize_adj_symmatrically(adj):
            rowsum = np.array(adj.sum(1))
        ...
        norm_adj_mat = normalized_adj_row(adj_mat + sp.eye(adj_mat.shape[0]))
        mean_adj_mat = normalized_adj_row(adj_mat)
        sym_adj_mat = normalize_adj_symmatrically(adj_mat)

        print('already normalize adjacency matrix', time() - t2)
        return adj_mat.tocsr(), norm_adj_mat.tocsr(), mean_adj_mat.tocsr(), sym_adj_mat.tocsr()
